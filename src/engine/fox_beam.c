...
#if ENABLE_60FPS == 1 // PlayerShot_DrawShot
void PlayerShot_DrawShot(PlayerShot* shot) {
    ...
    if (!isDrawn) {
        ...
        if ((shot->obj.id == PLAYERSHOT_TANK) || (shot->obj.id == PLAYERSHOT_ON_FOOT) || (shot->obj.id == PLAYERSHOT_7)) {
        ...
        switch (shot->obj.id) {
            ...
            case PLAYERSHOT_BOMB:
                if (shot->unk_5C == 0) {
                    ...
                    Matrix_RotateZ(gGfxMatrix, (gGameFrameCount DIV_FRAME_FACTOR) * 40.0f * M_DTOR, MTXF_APPLY);
            ...
            case PLAYERSHOT_TANK:
                ...
                if (gVersusMode) {
                    ...
                    RCP_SetupDL(&gMasterDisp, SETUPDL_49);
                    if (gLaserStrength[shot->sourceId] != LASERS_SINGLE) {
                        Matrix_RotateZ(gGfxMatrix, (gGameFrameCount DIV_FRAME_FACTOR) * 48.0f * M_DTOR, MTXF_APPLY);
                        Matrix_Push(&gGfxMatrix);
                        Matrix_Translate(gGfxMatrix, 0.f, 50.0f, 0.0f, MTXF_APPLY);
                        Matrix_RotateZ(gGfxMatrix, (gGameFrameCount DIV_FRAME_FACTOR) * 70.0f * M_DTOR, MTXF_APPLY);
                        Matrix_Scale(gGfxMatrix, 2.0f, 2.0f, 2.0f, MTXF_APPLY);
                        Matrix_SetGfxMtx(&gMasterDisp);
            ...
            case PLAYERSHOT_LOCK_ON:
                ...
                if (gCurrentLevel == LEVEL_AQUAS) {
                    ...
                    Matrix_RotateZ(gGfxMatrix, (gGameFrameCount DIV_FRAME_FACTOR) * 53.0f * M_DTOR, MTXF_APPLY);
                    Matrix_SetGfxMtx(&gMasterDisp);
                    gSPDisplayList(gMasterDisp++, D_101C2E0);
                    Matrix_RotateZ(gGfxMatrix, (gGameFrameCount DIV_FRAME_FACTOR) * -53.0f * 2.0f * M_DTOR, MTXF_APPLY);
                    Matrix_SetGfxMtx(&gMasterDisp);
                    gSPDisplayList(gMasterDisp++, D_101C2E0);
                    Matrix_Pop(&gGfxMatrix);
                    if (((gGameFrameCount % (2 MUL_FRAME_FACTOR)) == 0)) {

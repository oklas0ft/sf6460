...
void Actor_DrawEngineAndContrails(Actor* actor) {
    ...
    if ((actor->iwork[11] != 0) && (actor->obj.status == OBJ_ACTIVE)) {
        ...
        Matrix_Scale(gGfxMatrix, scale, scale * 0.7f, scale, MTXF_APPLY);
    ...
    scale = actor->fwork[21];
    if ((scale != 0.0f) && (gLevelType == LEVELTYPE_PLANET)) {
        sp54 = 0.0f;
        if ((gGameFrameCount % (2 MUL_FRAME_FACTOR)) != 0) {
            sp54 = 180.0f;
        }
        ...
        Matrix_Scale(gGfxMatrix, scale, 1.0f, 50.0f, MTXF_APPLY);
...
#if ENABLE_60FPS == 1 // Actor_DrawEngineGlow
void Actor_DrawEngineGlow(Actor* actor, s32 levelType) { // Engine Boss Meteo
    f32 scale;
    if ((actor->iwork[11] != 0) && (actor->obj.status == OBJ_ACTIVE)) {
        scale = 0.63f;
        if (actor->iwork[11] >= 2) {
        ...
        if ((gGameFrameCount % (2 MUL_FRAME_FACTOR)) != 0) {
            scale *= IMPROPER_DIV_FRAME_FACTOR(1.2f);
        }
        Matrix_Push(&gGfxMatrix);
        Matrix_Scale(gGfxMatrix, scale, scale, scale, MTXF_APPLY);
        Matrix_RotateZ(gGfxMatrix, -actor->obj.rot.z * M_DTOR, MTXF_APPLY);

...
void SectorX_8018F1DC(Actor* actor) {
    ...
    Actor_SpawnPlayerLaser(actor->index, actor->obj.pos.x + (dest.x * 1.5), actor->obj.pos.y + (dest.y * 1.5), actor->obj.pos.z + (dest.z * 1.5), dest.x, dest.y, dest.z, actor->rot_0F4.x, actor->rot_0F4.y + 180.0f, 0.0f);
}

#if ENABLE_60FPS == 1 // SectorX_8018F330 * Boss Second Wave
void SectorX_8018F330(Actor* actor) {
    f32 sp34;

    switch (actor->animFrame) {
        case 0:
            actor->obj.rot.x += 7.0f DIV_FRAME_FACTOR;
            actor->obj.rot.y += 5.0f DIV_FRAME_FACTOR;
            break;

        case 1:
            actor->rot_0F4.z += 3.0f DIV_FRAME_FACTOR;
            actor->obj.rot.z = SIN_DEG(actor->rot_0F4.z + 90.0f) * 60.0f;
            sp34 = fabsf(actor->obj.pos.z - gBosses[0].obj.pos.z) * 0.1f;
            Math_SmoothStepToF(&actor->obj.pos.x, gBosses[0].obj.pos.x + SIN_DEG(actor->rot_0F4.z) * sp34, 0.1f DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR,0.0f);
            Math_SmoothStepToF(&actor->obj.pos.y, gBosses[0].obj.pos.y, 0.1f DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->obj.pos.z, gBosses[0].obj.pos.z, 1.0f DIV_FRAME_FACTOR, 55.0f DIV_FRAME_FACTOR, 0);
            if (gBosses[0].state == 20) {
                actor->animFrame = 10;
                break;
            }

            if (((gGameFrameCount % (4 MUL_FRAME_FACTOR)) == 0) && (Rand_ZeroOne() < 0.2f)) {
        ...
        case 2:
            ...
            actor->obj.rot.x += 5.5f DIV_FRAME_FACTOR;
            actor->obj.rot.y += 5.0f DIV_FRAME_FACTOR;
            ...
            Math_SmoothStepToF(&actor->fwork[0], 0.5f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor->fwork[1], 0.1f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&gPlayer[0].cam.at.x, actor->obj.pos.x, actor->fwork[0] DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&gPlayer[0].cam.at.y, actor->obj.pos.y, actor->fwork[0] DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&gPlayer[0].cam.at.z, actor->obj.pos.z + gPathProgress, actor->fwork[0] DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&gPlayer[0].cam.eye.x, actor->obj.pos.x - 30.0f, actor->fwork[1] DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&gPlayer[0].cam.eye.y, actor->obj.pos.y, actor->fwork[1] DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR, 0.0f);
        ...
        case 10:
            Math_SmoothStepToF(&actor->obj.rot.y, 90.0f, 0.1f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->obj.rot.z, 90.0f, 1.0f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->obj.pos.x, gBosses[0].obj.pos.x + 10000.0f, 0.1f DIV_FRAME_FACTOR, 40.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->obj.pos.y, gBosses[0].obj.pos.y + 5000.0f, 0.1f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->obj.pos.z, gBosses[0].obj.pos.z, 1.0f DIV_FRAME_FACTOR, 55.0f DIV_FRAME_FACTOR, 0.0f);
            break;
    }
}
#else
...
#if ENABLE_60FPS == 1 // SectorX_80190078
void SectorX_80190078(Boss* boss) {
    ...
    if (boss->swork[7] != 0) {
        ...
        Math_SmoothStepToAngle(&boss->fwork[4], sp70, 0.1f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        Math_SmoothStepToAngle(&boss->fwork[6], sp6C, 0.1f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
    } else {
        Math_SmoothStepToF(&boss->fwork[4], 182.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        Math_SmoothStepToAngle(&boss->fwork[6], 0.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
    }

    boss->fwork[5] = SIN_DEG(boss->timer_054 * 50.0f) * boss->timer_054;
    boss->fwork[7] = SIN_DEG(boss->timer_054 * 30.0f) * boss->timer_054 * 0.5f;

    Math_SmoothStepToF(&boss->obj.pos.z, gPlayer[0].pos.z + boss->fwork[1], 0.1f DIV_FRAME_FACTOR, boss->fwork[42] DIV_FRAME_FACTOR, 0);
    Math_SmoothStepToF(&boss->obj.pos.y, boss->fwork[2], 0.05f DIV_FRAME_FACTOR, boss->fwork[43] DIV_FRAME_FACTOR, 0);
    Math_SmoothStepToF(&boss->obj.pos.x, boss->fwork[3], 0.05f DIV_FRAME_FACTOR, boss->fwork[44] DIV_FRAME_FACTOR, 0);
    Math_SmoothStepToAngle(&boss->obj.rot.z, boss->fwork[41], 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);

    if (boss->state != 5) {
        Math_SmoothStepToF(&boss->fwork[29], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&boss->fwork[30], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&boss->fwork[31], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&boss->fwork[32], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&boss->fwork[33], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&boss->fwork[34], 0.f, 0.1f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
        if (boss->state != 4) {
            Math_SmoothStepToF(&boss->fwork[27], 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[28], 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
        }
    }
    ...
    if (boss->dmgType != DMG_NONE) {
        ...
        if (boss->swork[0] == 1) {
            if ((boss->dmgPart == 0) && ((boss->fwork[4] < 45.0f) || (boss->fwork[4] > 315.0f))) {
                ...
                boss->timer_054 = 20;
                func_effect_8007C120(boss->obj.pos.x, boss->obj.pos.y + 334.0f, -237.0f + boss->obj.pos.z, boss->vel.x, boss->vel.y, boss->vel.z, 0.2f, 20);

                if (boss->swork[3] <= 0) {
                    func_effect_8007BFFC(boss->obj.pos.x, boss->obj.pos.y + 300.0f, boss->obj.pos.z, 0.0f, 0.0f, 0.0f, 10.0f, 50);

                    func_effect_8007C120(boss->obj.pos.x, boss->obj.pos.y + 334.0f, -237.0f + boss->obj.pos.z, boss->vel.x, boss->vel.y, boss->vel.z, 0.15f, 70);
                    for (i = 0; i < 10; i++) {
        ...
        if (boss->swork[0] == 2) {
            if (boss->dmgPart == 0) {
                ...
                func_effect_8007C120(boss->obj.pos.x, boss->obj.pos.y, boss->obj.pos.z, boss->vel.x, boss->vel.y, boss->vel.z, 0.2f, 10);
            } else {
                func_effect_8007A6F0(&boss->obj.pos, NA_SE_EN_REFLECT);
            }
        }
    ...
    switch (boss->state) {
        ...
        case 90:
            ...
            //left arm
            if (fabsf(Math_SmoothStepToF(&boss->fwork[16], 0.0f, 0.1f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0)) < 8.0f DIV_FRAME_FACTOR) { //??????
                Math_SmoothStepToF(&boss->fwork[14], 0.0f, 0.2f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0);
                if ((boss->timer_050 % (8)) == 0) {
                    func_effect_8007C120(boss->obj.pos.x + 312.0f, boss->obj.pos.y, boss->obj.pos.z - 173.0f, boss->vel.x, boss->vel.y, boss->vel.z, 0.2f, 8);
                }
            }
            // Right arm
            if (fabsf(Math_SmoothStepToF(&boss->fwork[19], 0.0f, 0.1f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0)) < 8.0f DIV_FRAME_FACTOR) { //??????
                Math_SmoothStepToF(&boss->fwork[17], 0.0f, 0.2f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0);
                if ((boss->timer_050 % 8) == 0) {
                    func_effect_8007C120(boss->obj.pos.x - 312.0f, boss->obj.pos.y, boss->obj.pos.z - 173.0f, boss->vel.x, boss->vel.y, boss->vel.z, 0.2f, 8);
                }
            }
        ...
        case 100:
            ...
            Math_SmoothStepToVec3fArray(sp80, boss->vwork, 1, frameData, boss->fwork[0] DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.0f);

            Math_SmoothStepToF(&boss->fwork[0], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR, 0);
            if (((gGameFrameCountHack % FRAME_FACTOR) == 0)) { // 60fps HACK
            boss->unk_04C++;
            }
        ...
        case 2: // right arm attack
            ...
            if (boss->unk_04C > 60) {
                ...
                Math_SmoothStepToF(&boss->fwork[42], 100.0f, 1.0f DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR, 0);
                boss->fwork[43] = boss->fwork[44] = 10.0f;
            } else {
                boss->fwork[1] = -2000.0f;
                Math_SmoothStepToF(&boss->fwork[42], 30.0f, 1.0f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0);
                boss->fwork[43] = boss->fwork[44] = 10.0f;
            }
        ...
        case 4: // Arms launch setup
            ...
            Math_SmoothStepToF(&boss->fwork[44], 10.0f, 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[43], 10.0f, 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[42], 50.0f, 1.0f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0);

            frameData = Animation_GetFrameData(&D_SX_6016E28, 0, sp80);
            Math_SmoothStepToVec3fArray(sp80, boss->vwork, 1, frameData, boss->fwork[0]DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR , 0.0f);

            Math_SmoothStepToF(&boss->fwork[0], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[27], 30.0f, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[28], -30.0f, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
        ...
        case 5: // arms lauching attack
            ...
            if ((boss->obj.pos.x + boss->fwork[8]) < gPlayer[0].pos.x) {
                boss->fwork[29] += sp60 DIV_FRAME_FACTOR;
            }
            ...
            if ((boss->obj.pos.y + boss->fwork[9]) < gPlayer[0].pos.y) {
                boss->fwork[30] += sp60 DIV_FRAME_FACTOR ;
            }
            ...
            if ((boss->obj.pos.x + boss->fwork[11]) < gPlayer[0].pos.x) {
                boss->fwork[32] += sp60 DIV_FRAME_FACTOR ;
            }
            ...
            if ((boss->obj.pos.y + boss->fwork[12]) < gPlayer[0].pos.y) {
                boss->fwork[33] += sp60 DIV_FRAME_FACTOR ;
            }
            ...
            Math_SmoothStepToF(&boss->fwork[31], 4000.0f, 1.0f DIV_FRAME_FACTOR, 80.0f DIV_FRAME_FACTOR, 0);

            if (boss->timer_050 < 70) {
                Math_SmoothStepToF(&boss->fwork[34], 4000.0f, 1.0f DIV_FRAME_FACTOR, 80.0f DIV_FRAME_FACTOR, 0);
            }
        ...
        case 75:
            ...
            Math_SmoothStepToVec3fArray(sp80, boss->vwork, 1, frameData, boss->fwork[0] DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0.0f);

            Math_SmoothStepToF(&boss->fwork[0], 0.1f, 1.0f DIV_FRAME_FACTOR, 0.005f DIV_FRAME_FACTOR, 0);
        ...
        case 8:
            ...
            Math_SmoothStepToF(&boss->fwork[0], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.02f DIV_FRAME_FACTOR, 0);
        ...
        case 85:
            ...
            frameData = Animation_GetFrameData(&D_SX_60123BC, boss->unk_04C, sp80);
            Math_SmoothStepToVec3fArray(sp80, boss->vwork, 1, frameData, boss->fwork[0] DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0.0f);

            Math_SmoothStepToF(&boss->fwork[0], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0);
        ...
        case 35:
            ...
            Math_SmoothStepToF(&boss->fwork[42], 60.0f, 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[44], 20.0f, 1.0f DIV_FRAME_FACTOR, 0.5f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->fwork[43], 20.0f, 1.0f DIV_FRAME_FACTOR, 0.5f DIV_FRAME_FACTOR, 0);

            frameData = Animation_GetFrameData(&D_SX_6009FF8, 0, sp80);
            Math_SmoothStepToVec3fArray(sp80, boss->vwork, 1, frameData, boss->fwork[0] DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f);

            Math_SmoothStepToF(&boss->fwork[0], 0.1f, 1.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->rot_078.y, 10.0f, 1.0f DIV_FRAME_FACTOR, 0.1f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&boss->obj.rot.y, 2880.0f, 0.1f DIV_FRAME_FACTOR, boss->rot_078.y DIV_FRAME_FACTOR, 0.1f DIV_FRAME_FACTOR);
        ...
        case 20:
            Math_SmoothStepToF(&boss->obj.rot.y, 0.0f, 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.1f DIV_FRAME_FACTOR);
            boss->swork[0] = 0;
            boss->fwork[1] = -1800.0f;
            ...
            if ((((gGameFrameCount % (8 MUL_FRAME_FACTOR)) == 0)) && (Rand_ZeroOne() < 0.5f)) {
                boss->timer_05C = 4;
            }
            ...
            if (boss->timer_050 < 230) {
                Math_SmoothStepToF(&boss->fwork[14], 10000.0f, 1.0f DIV_FRAME_FACTOR, 60.0f DIV_FRAME_FACTOR, 0);
                Math_SmoothStepToF(&boss->fwork[17], -10000.0f, 1.0f DIV_FRAME_FACTOR, 60.0f DIV_FRAME_FACTOR, 0);
            }
    ...
    if (((boss->swork[1] != 0) && (boss->swork[3] > 0)) && ((boss->fwork[4] < 45.0f) || (boss->fwork[4] > 315.0f))) {
        if (((gGameFrameCountHack % FRAME_FACTOR) == 0)) { // 60fps HACK *blue shots timing
        boss->swork[1]++;
        }
        if (boss->swork[1] > 40) {
            if (boss->swork[1] > 50) {
    ...
    if (boss->swork[0] == 1) {
        Math_SmoothStepToF(&boss->fwork[45], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR, 0);
    } else {
        Math_SmoothStepToF(&boss->fwork[45], 0.0f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR, 0);
    }
}
#else
void SectorX_80190078(Boss* boss) {
...
#if ENABLE_60FPS == 0 // SectorX_80192AF0
bool SectorX_80192AF0(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* data) {
    ...
    switch (limbIndex) {
        case 1:
            rot->x += boss->fwork[4] + boss->fwork[5] DIV_FRAME_FACTOR;
            rot->z += boss->fwork[6] + boss->fwork[7] DIV_FRAME_FACTOR;
        ...
        case 34:
            rot->x += boss->fwork[27] DIV_FRAME_FACTOR;
            break;

        case 33:
            pos->z -= boss->fwork[31] DIV_FRAME_FACTOR;
            pos->y -= boss->fwork[29] DIV_FRAME_FACTOR;
            pos->x -= boss->fwork[30] DIV_FRAME_FACTOR;
            break;

        case 38:
            rot->x += boss->fwork[28] DIV_FRAME_FACTOR;
            break;

        case 37:
            pos->z += boss->fwork[34] DIV_FRAME_FACTOR;
            pos->y += boss->fwork[32] DIV_FRAME_FACTOR;
            pos->x -= boss->fwork[33] DIV_FRAME_FACTOR;
            break;

        case 35:
            pos->x -= boss->fwork[14] DIV_FRAME_FACTOR;
            pos->y -= boss->fwork[16] DIV_FRAME_FACTOR;
            pos->z -= boss->fwork[15] DIV_FRAME_FACTOR;
            if (boss->state == 20) {
                rot->x += (boss->fwork[14] * 0.2f) DIV_FRAME_FACTOR;
                rot->y += (boss->fwork[14] * 0.1f) DIV_FRAME_FACTOR;
            }
            break;

        case 39:
            pos->x += boss->fwork[17] DIV_FRAME_FACTOR;
            pos->y -= boss->fwork[19] DIV_FRAME_FACTOR;
            pos->z += boss->fwork[18] DIV_FRAME_FACTOR;
            if (boss->state == 20) {
                rot->x += (boss->fwork[17] * 0.1f) DIV_FRAME_FACTOR;
                rot->z += (boss->fwork[17] * 0.2f) DIV_FRAME_FACTOR;
            }
            break;
    }
...
#if ENABLE_60FPS == 1 // SectorX_80193434
void SectorX_80193434(Boss* boss) {
    ...
    if (boss->swork[5] == 0) {
        ...
        if (boss->fwork[31] > 10.0f) {
            ...
            if ((gGameFrameCount % (2 MUL_FRAME_FACTOR)) != 0) {
                fwork *= IMPROPER_DIV_FRAME_FACTOR(1.2f);
            }
            Matrix_Push(&gGfxMatrix);
...
#if ENABLE_60FPS == 1 // SectorX_LevelStart
void SectorX_LevelStart(Player* player) {
    ...
    switch (player->csState) {
        ...
        case 1:
            ...
            player->camRoll -= 0.1f DIV_FRAME_FACTOR;
            ...
            if (gCsFrameCount == (143 MUL_FRAME_FACTOR)) {
                Object_Kill(&gPlayerShots[0].obj, gPlayerShots[0].sfxSource);
                func_effect_8007BFFC(gActors[5].obj.pos.x, gActors[5].obj.pos.y, gActors[5].obj.pos.z, 0.0f, 0.0f, 0.0f, 3.0f, 40);
        ...
        case 3:
            if (gCsFrameCount > (200 MUL_FRAME_FACTOR)) {
                ...
                Math_SmoothStepToF(D_ctx_80177A48, 0.1f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0);
            }

            Math_SmoothStepToF(&player->camRoll, 0.0f, 0.5f DIV_FRAME_FACTOR, 0.07f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.z, 0.0f, 0.2f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.x, 20.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
        ...
        case 4:
            ...
            Math_SmoothStepToF(&player->camRoll, 0.0f, 0.2f DIV_FRAME_FACTOR, 0.5f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->pos.y, 350.0f, 0.1f DIV_FRAME_FACTOR, 15.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.x, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            ...
            Math_SmoothStepToF(D_ctx_80177A48, 1.0f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0);
...
#if ENABLE_60FPS == 1 // SectorX_LevelComplete
void SectorX_LevelComplete(Player* player) {
    ...
    switch (player->csState) {
        ...
        case 1:
            if (gCsFrameCount > (990 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(D_ctx_80177A48 + 3, 0.0f, 0.1f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0.0f);
            } else if (gCsFrameCount > (120 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(D_ctx_80177A48 + 3, -0.5f, 0.1f DIV_FRAME_FACTOR, 0.02f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(D_ctx_80177A48 + 4, 300.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(D_ctx_80177A48 + 5, -50.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0f);
            }
            D_ctx_80177A48[2] += D_ctx_80177A48[3] DIV_FRAME_FACTOR;
            Math_SmoothStepToF(&player->baseSpeed, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(D_ctx_80177A48, 0.1f, 1.0f DIV_FRAME_FACTOR, 0.002f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(D_ctx_80177A48 + 1, 0.1f, 1.0f DIV_FRAME_FACTOR, 0.002f DIV_FRAME_FACTOR, 0.0f);
            Matrix_RotateY(gCalcMatrix, player->rot.y * M_DTOR, MTXF_NEW);
            Matrix_RotateX(gCalcMatrix, player->rot.x * M_DTOR, MTXF_APPLY);
            Matrix_Push(&gCalcMatrix);
            ...
            Math_SmoothStepToF(&player->cam.eye.x, player->pos.x + sp48.x, D_ctx_80177A48[0] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.eye.y, player->pos.y + sp48.y, D_ctx_80177A48[0] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.eye.z, player->trueZpos + gPathProgress + sp48.z, D_ctx_80177A48[0] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Matrix_Pop(&gCalcMatrix);
            sp54.x = 0.0f;
            sp54.y = 0;
            sp54.z = D_ctx_80177A48[5];
            Matrix_MultVec3f(gCalcMatrix, &sp54, &sp48);
            Math_SmoothStepToF(&player->cam.at.x, player->pos.x + sp48.x, D_ctx_80177A48[1] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.y, player->pos.y + sp48.y, D_ctx_80177A48[1] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.z, player->trueZpos + gPathProgress + sp48.z, D_ctx_80177A48[1] DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            break;

        case 2:
            player->baseSpeed += 2.0f DIV_FRAME_FACTOR;
            if (player->csTimer == 0) {
                Math_SmoothStepToAngle(&player->rot.x, 20.0f, 0.1f DIV_FRAME_FACTOR, 0.5f DIV_FRAME_FACTOR, 0.0f);
            }

            if (gCsFrameCount == (1195 MUL_FRAME_FACTOR)) {
                Audio_FadeOutAll(10);
            }

    ...
    if (gPlayer[0].state_1C8 != PLAYERSTATE_1C8_START_360) {
        ...
        if (((gGameFrameCount % 2) == 0)) { // 60fps HACK
            ...
            if (boss->state < 5) {
                ...
                Math_SmoothStepToF(&boss->obj.rot.z, sp5C, 0.2f DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            ...
            Math_SmoothStepToAngle(&D_i1_8019B6D8[16], sp1FC, 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);
            Math_SmoothStepToAngle(&D_i1_8019B6D8[15], sp200, 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);

            if (boss->state != 0) {
                sp1F8 = Math_RadToDeg(
                    Math_Atan2F(D_i1_8019B6D8[66] - boss->obj.pos.x, D_i1_8019B6D8[67] - boss->obj.pos.z));
                Math_SmoothStepToAngle(&boss->obj.rot.y, sp1F8, 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);
            }
            ...
            switch (boss->state) {
                ...
                case 1:
                    ...
                    boss->unk_04C += 2 DIV_FRAME_FACTOR;
                ...
                case 6:
                    ...
                    boss->fwork[12] += 0.05f DIV_FRAME_FACTOR;

                    if (boss->state == 6) {
                        boss->obj.rot.z += boss->fwork[12] DIV_FRAME_FACTOR;
                    ...
                    if (boss->timer_050 == 0) {
                        ...
                        if (objId != 0) {
                            Corneria_80187710(D_i1_8019B6D8[62], D_i1_8019B6D8[63], D_i1_8019B6D8[64], 65.0f, 270.0f, 0.0f, 0, 0, objId);
                        }
                    }
                ...
                case 7:
                    if (boss->swork[32] != 0) {
                        boss->fwork[12] += 0.05f DIV_FRAME_FACTOR;
                        boss->obj.rot.x += boss->fwork[12] DIV_FRAME_FACTOR;
                    ...
                    if (boss->timer_050 == 10) {
                        ...
                        for (sp218 = 0; sp218 < 100; sp218++) {
                            func_effect_80079618(D_i1_8019B6D8[62] + RAND_FLOAT_CENTERED(600.0f), (D_i1_8019B6D8[63] + RAND_FLOAT_CENTERED(500.0f)) - 100.0f, D_i1_8019B6D8[64] + RAND_FLOAT_CENTERED(600.0f), 2.0f);
                        }
                    }
            ...
            Math_SmoothStepToF(&boss->vel.x, sp21C.x, 0.3f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);
            Math_SmoothStepToF(&boss->vel.z, sp21C.z, 0.3f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);

            Corneria_801887AC(boss);

            Math_SmoothStepToVec3fArray(sp84, boss->vwork, 1, 19, boss->fwork[14] DIV_FRAME_FACTOR, 100.0f DIV_FRAME_FACTOR, 0.0f DIV_FRAME_FACTOR);
            Math_SmoothStepToF(&boss->fwork[14], 1.0f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0.0f);
...
bool Corneria_8018AB08(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* data) {
    ...
    if (limbIndex == 1) {
        rot->x += actor->fwork[1] DIV_FRAME_FACTOR;
    }
    if ((limbIndex == 3) && (actor->obj.id == OBJ_ACTOR_CO_GARUDA_1)) {
        rot->x += actor->fwork[1] DIV_FRAME_FACTOR; // rotation before the punch
    }
    return false;
}
#else
...
void Corneria_8018ACE0(Actor* actor) {
    if (actor->dmgType != DMG_NONE) {
        ...
        if (actor->dmgPart == 0) {
            ...
            func_effect_8007C120(actor->obj.pos.x, actor->obj.pos.y + 200.0f, actor->obj.pos.z, actor->vel.x, actor->vel.y, actor->vel.z, 0.1f, 20);
            actor->timer_0C6 = 15;
...
void Corneria_8018AED0(Actor* actor) {
    ...
    // this is it ?
    Math_SmoothStepToVec3fArray(sp40, actor->vwork, 0, Animation_GetFrameData(&D_CO_602991C, actor->animFrame, sp40), 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR);
...
void Corneria_8018B15C(Actor* actor) {
    ...
    switch (actor->state) {
        ...
        case 1:
            ...
            if (actor->timer_0BC == 0) {
                actor->state = 2;
                actor->iwork[2] = (RAND_INT(10.0f) + 10) MUL_FRAME_FACTOR;
            }
            break;
    ...
    Math_SmoothStepToVec3fArray(sp60, actor->vwork, 0, Animation_GetFrameData(&D_CO_602AA04, actor->animFrame, sp60), 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR); // 60fps
}
#else
void Corneria_8018B15C(Actor* actor) {
    Vec3f sp60[20];
...
void Corneria_8018B418(Actor* actor) {
    ...
    switch (actor->state) {
        ...
        case 1:
            actor->fwork[0] = 5.0f;
            actor->fwork[1] += 5.0f DIV_FRAME_FACTOR;
            Texture_Scroll(D_CO_60329C0, 16, 16, 1);
            actor->animFrame++;
    ...
    switch (actor->state) {
        ...
        case 101:
            if ((actor->timer_0BE != 0) && ((gGameFrameCount % 2) == 0)) {
                func_effect_8007797C(actor->obj.pos.x, actor->obj.pos.y + 150.0f, actor->obj.pos.z, 0.0f, 10.0f , 0.0f, 3.0f);
            }
            break;
    }
}
...
#if ENABLE_60FPS == 1 // Corneria_8018C19C *ATTACK CARRIER Update
void Corneria_8018C19C(Boss* boss) { // ATTACK CARRIER Update
  ...
  // }
    ...
    if (D_i1_8019B6D0 == 0) {
        ...
        if (((gGameFrameCount % (2 MUL_FRAME_FACTOR)) == 0)) {
            ...
            func_effect_8007BC7C(sp84[6].x + boss->obj.pos.x, sp84[6].y + boss->obj.pos.y, sp84[6].z + boss->obj.pos.z, 20.0f);
            func_effect_8007BC7C(sp84[7].x + boss->obj.pos.x, sp84[7].y + boss->obj.pos.y, sp84[7].z + boss->obj.pos.z, 10.0f);
            func_effect_8007BC7C(sp84[8].x + boss->obj.pos.x, sp84[8].y + boss->obj.pos.y, sp84[8].z + boss->obj.pos.z, 10.0f);
        }
    } else {
        ...
        if (boss->state != 7) {
            Math_SmoothStepToF(&boss->fwork[4], boss->fwork[5], 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
            Math_SmoothStepToF(&boss->obj.pos.z, boss->fwork[3], 0.1f DIV_FRAME_FACTOR, boss->fwork[4] DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        }

        Math_SmoothStepToF(&boss->obj.pos.y, boss->fwork[6], 0.02f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        Math_SmoothStepToAngle(&boss->fwork[0], boss->fwork[9], 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        Math_SmoothStepToAngle(&boss->fwork[1], boss->fwork[10], 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        Math_SmoothStepToAngle(&boss->fwork[2], boss->fwork[11], 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);

        if (boss->state < 6) {
            Math_SmoothStepToAngle(&boss->obj.rot.y, boss->fwork[13], 0.03f DIV_FRAME_FACTOR, boss->fwork[8] DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
            Math_SmoothStepToF(&boss->fwork[8], boss->fwork[12], 0.1f DIV_FRAME_FACTOR, 0.02f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            Math_SmoothStepToAngle(&boss->obj.rot.z, boss->fwork[14], 0.03f DIV_FRAME_FACTOR, boss->fwork[15] DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
        ...
        if (boss->timer_052 != 0) {
            Math_SmoothStepToF(&D_ctx_801779A8[0], 50.0f, 1.0f DIV_FRAME_FACTOR, 1.6f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
        }
        ...
        switch (boss->state) {
            ...
            case 6:
                Math_SmoothStepToAngle(&boss->obj.rot.y, boss->fwork[13], 0.1f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR);
                Math_SmoothStepToAngle(&boss->obj.rot.z, boss->fwork[14], 0.1f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR);
            ...
            case 7:
                ...
                Math_SmoothStepToF(&boss->obj.pos.z, boss->fwork[3], 0.1f DIV_FRAME_FACTOR, 15.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                ...
                if (boss->health < 2) {
                    ...
                    Math_SmoothStepToF(&boss->fwork[8], 3.0f, 0.1f DIV_FRAME_FACTOR, 0.5f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                    Math_SmoothStepToAngle(&boss->obj.rot.z, boss->fwork[14], 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);

                    temp_fv0_2 = Math_SmoothStepToF(&boss->obj.rot.y, boss->fwork[20], 0.1f DIV_FRAME_FACTOR, boss->fwork[8] DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                    boss->fwork[14] = 0.0f;
                    if (temp_fv0_2 < -1.0f) {
                        boss->fwork[14] = 25.0f;
                    }
            ...
            case 8:
                ...
                Math_SmoothStepToF(&boss->obj.pos.x, boss->fwork[7], 1.0f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&boss->vel.x, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&boss->vel.y, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&boss->vel.z, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                ...
                Math_SmoothStepToAngle(&boss->obj.rot.z, boss->fwork[14], 1.0f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                ...
                if (boss->health != 0) {
                    if (boss->timer_056 == 0) {
                        ...
                        Math_SmoothStepToF(&boss->obj.rot.y, 0.0f, 0.1f DIV_FRAME_FACTOR, boss->fwork[8] DIV_FRAME_FACTOR, 0.00001f DIV_FRAME_FACTOR);
                        sp78.x = 0.0f;
                        sp78.y = 0.0f;
                        sp78.z = 20.0f;
                        Matrix_MultVec3f(gCalcMatrix, &sp78, &sp6C);
...
void Corneria_8018F678(void) {
    ...
    if (((gGameFrameCount % (32 MUL_FRAME_FACTOR)) == 0) && gPlayer[0].pos.x == 0.0f) {
...
#if ENABLE_60FPS == 1 // Corneria_LevelStart *Cutscene

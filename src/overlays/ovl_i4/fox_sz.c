...
#if ENABLE_60FPS == 1 // SectorZ_Missile_Update
void SectorZ_Missile_Update(Actor* this) {
    ...
    SIN_DEG(gGameFrameCount DIV_FRAME_FACTOR); // Leftover from a test or commented out code?
    ...
    if (this->aiType < 100) {
        ...
        sp20 = SIN_DEG((this->index * 45) + (gGameFrameCount DIV_FRAME_FACTOR)) * 5000.0f;
        sp1C = COS_DEG((this->index * 45) + ((gGameFrameCount DIV_FRAME_FACTOR) * 2)) * 5000.0f;
    } else {
        sp20 = sp1C = 0.0f;
    }
...
#if ENABLE_60FPS == 1 // SectorZ_UpdateEvents
void SectorZ_UpdateEvents(Actor* actor) {
    ...
    switch ((s32) actor->state) {
        ...
        case 3:
            ...
            player->cam.eye.x += (actor8->vel.x * 0.23f) DIV_FRAME_FACTOR;
            player->cam.eye.y += (actor8->vel.y * 0.23f) DIV_FRAME_FACTOR;
            player->cam.eye.z += (actor8->vel.z * 0.23f) DIV_FRAME_FACTOR;

            Math_SmoothStepToF(&player->cam.at.x, actor8->obj.pos.x, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->cam.at.y, actor8->obj.pos.y, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->cam.at.z, actor8->obj.pos.z, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->camRoll, 0, 1.0f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0);
        ...
        case 10:
            ...
            Math_SmoothStepToF(&player->cam.at.x, gActors[10].obj.pos.x, 0.2 DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.y, gActors[10].obj.pos.y, 0.2 DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.z, gActors[10].obj.pos.z, 0.2 DIV_FRAME_FACTOR, 500.0f DIV_FRAME_FACTOR, 0.0f);

            if (actor->timer_0BC < 9800) {
                Math_SmoothStepToF(&gActors[10].fwork[1], 80.0f, 0.1 DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0);
                Math_SmoothStepToF(&gActors[10].fwork[29], 3.0f, 0.1 DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            } else {
                Math_SmoothStepToF(&actor->fwork[10], 700.0f, 1 DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(&player->cam.eye.x, gActors[10].obj.pos.x - 300.0f, 0.07f DIV_FRAME_FACTOR, actor->fwork[10] DIV_FRAME_FACTOR, 0);
                Math_SmoothStepToF(&player->cam.eye.y, gActors[10].obj.pos.y, 0.07f DIV_FRAME_FACTOR, actor->fwork[10] DIV_FRAME_FACTOR, 0);
                Math_SmoothStepToF(&player->cam.eye.z, gActors[10].obj.pos.z + 500.0f, 0.05f DIV_FRAME_FACTOR, 50.0f DIV_FRAME_FACTOR, 0);
                Math_SmoothStepToF(&gActors[10].fwork[29], 2.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            }
    ...
    if ((gGameFrameCount & 0x18) == 0) {
        Math_SmoothStepToF(&gSzMissileR, 63.0f, 1.0f DIV_FRAME_FACTOR, 9.450001f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&gSzMissileG, 255.0f, 1.0f DIV_FRAME_FACTOR, 35.25f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&gSzMissileB, 158.0f, 1.0f DIV_FRAME_FACTOR, 21.6f DIV_FRAME_FACTOR, 0);
    } else {
        Math_SmoothStepToF(&gSzMissileR, 0.0f, 1.0f DIV_FRAME_FACTOR, 3.15f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&gSzMissileG, 20.0f, 1.0f DIV_FRAME_FACTOR, 11.75f DIV_FRAME_FACTOR, 0);
        Math_SmoothStepToF(&gSzMissileB, 14.0f, 1.0f DIV_FRAME_FACTOR, 7.2000003f DIV_FRAME_FACTOR, 0);
    }
}
#else
void SectorZ_UpdateEvents(Actor* actor) {
...
#if ENABLE_60FPS == 1 // SectorZ_LevelStart
...
#if ENABLE_60FPS == 1 // SectorZ_LevelComplete
void SectorZ_LevelComplete(Player* player) {
    ...
    Math_SmoothStepToF(&player->boostSpeed, 0.0f, 0.1f DIV_FRAME_FACTOR, 1.5f DIV_FRAME_FACTOR, 0.0f);

    switch (player->csState) {
        case 1000:
            Math_SmoothStepToF(&player->rot.y, -40.0f, 0.1f DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.z, -60.0f, 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.x, 0, 0.1f DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0);
            if (player->csTimer == 0) {
                player->csState = 1001;
                player->csTimer = 100;
                AUDIO_PLAY_SFX(NA_SE_ARWING_BOOST, player->sfxSource, 0);
        ...
        case 1001:
            ...
            Math_SmoothStepToF(&player->rot.x, 15.0f, 0.1f DIV_FRAME_FACTOR, 0.4f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.z, 40.0f, 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&player->rot.y, 120.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);

            player->baseSpeed += 1.0f DIV_FRAME_FACTOR;
        ...
        case 1:
            ...
            Math_SmoothStepToAngle(&boss0->rot_078.x, 20.0f, 0.03f DIV_FRAME_FACTOR, boss0->fwork[1] DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToAngle(&boss0->rot_078.y, 180.0f, 0.03f DIV_FRAME_FACTOR, boss0->fwork[2] DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToAngle(&boss0->rot_078.z, 30.0f, 0.03f DIV_FRAME_FACTOR, boss0->fwork[3] DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&boss0->fwork[0], 20.0f, 0.05f DIV_FRAME_FACTOR, 0.3f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&boss0->fwork[1], 0.07f, 1.0f DIV_FRAME_FACTOR, 0.07f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&boss0->fwork[2], 0.5f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&boss0->fwork[3], 0.7f, 1.0f DIV_FRAME_FACTOR, 0.7f DIV_FRAME_FACTOR, 0.0f);

            if ((gCsFrameCount == (100 MUL_FRAME_FACTOR)) && gGreatFoxIntact) {
                AUDIO_PLAY_BGM(NA_BGM_COURSE_CLEAR);
            }

            if ((gCsFrameCount == (210 MUL_FRAME_FACTOR)) && gGreatFoxIntact) {
                gLevelClearScreenTimer = 100;
                gMissionStatus = MISSION_ACCOMPLISHED;
            }

            if ((gCsFrameCount == (230 MUL_FRAME_FACTOR)) && (D_i4_801A0564 != 0)) {
                SectorZ_8019C70C();
            }
        ...
        case 2:
            ...
            if (player->csTimer == 0) {
                ...
                gCsFrameCount = (1280 MUL_FRAME_FACTOR);
        ...
        case 10:
            ...
            if (gCsFrameCount == (1620 MUL_FRAME_FACTOR)) {
                player->csState++;
                D_ctx_80177A48[0] = 0.0f;
        ...
        case 11:
            ...
            D_ctx_80177A48[1] += (D_ctx_80177A48[2] * var_fv1) DIV_FRAME_FACTOR;
            ...
            Math_SmoothStepToF(&player->baseSpeed, 0, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor1->fwork[0], 0, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor3->fwork[0], 0, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor2->fwork[0], 0.0f, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor0->fwork[0], 0.0f, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(D_ctx_80177A48, 0.02f, 1.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR, 0);

            if (gCsFrameCount > (2000 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(&D_ctx_80177A48[3], 50.0f, 0.05f DIV_FRAME_FACTOR, 1.5f DIV_FRAME_FACTOR, 0);
            }

            if (gCsFrameCount < (2330 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(&D_ctx_80177A48[2], 0.5f, 1.0f DIV_FRAME_FACTOR, 0.008f DIV_FRAME_FACTOR, 0);
            } else {
                Math_SmoothStepToF(&D_ctx_80177A48[2], 0.0f, 1.0f DIV_FRAME_FACTOR, 0.002f DIV_FRAME_FACTOR, 0);
            }

            if (gCsFrameCount > (2510 MUL_FRAME_FACTOR)) {
                player->baseSpeed += 2.0f DIV_FRAME_FACTOR;
                player->rot.x += 0.1f DIV_FRAME_FACTOR;
                player->unk_190 = 2.0f;
            }

            if (gCsFrameCount > (2550 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(&D_ctx_80177A48[0], 0.1f, 1.0f DIV_FRAME_FACTOR, 0.005f DIV_FRAME_FACTOR, 0);
            } else {
            ...
            if (gCsFrameCount == (2660 MUL_FRAME_FACTOR)) {
                Audio_FadeOutAll(50);
            }

            if (gCsFrameCount > (2710 MUL_FRAME_FACTOR)) {
                gFillScreenAlphaTarget = 255;
                gFillScreenRed = gFillScreenGreen = gFillScreenBlue = 0;
    ...
    if (player->csState >= 1000) {
        if (player->pos.y < 700.0f) {
            Math_SmoothStepToF(&player->pos.y, 700.0f, 0.1f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.0f);
        }
        Camera_Update360(player, 0);
        player->cam.eye.x += (player->vel.x * 0.1f) DIV_FRAME_FACTOR;
        player->cam.eye.y += (player->vel.y * 0.1f) DIV_FRAME_FACTOR;
        player->cam.eye.z += (player->vel.z * 0.1f) DIV_FRAME_FACTOR;
    } else {
...
#if ENABLE_60FPS == 1 // SectorZ_8019DD20
void SectorZ_8019DD20(Actor* actor) {
    ...
    switch (actor->state) {
        ...
        case 2:
            ...
            actor->rot_0F4.x += actor->fwork[1] DIV_FRAME_FACTOR;
            Math_SmoothStepToF(&actor->fwork[1], 0.1f, 1.0f DIV_FRAME_FACTOR, 0.01f DIV_FRAME_FACTOR, 0.0f);
            break;
        ...
        case 11:
            Math_SmoothStepToF(&actor->rot_0F4.x, 215.0f, 0.1f DIV_FRAME_FACTOR, 7.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->fwork[0], 10.0f, 0.1f DIV_FRAME_FACTOR, 1.5f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToAngle(&actor->rot_0F4.y, 140.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0f);
        ...
        case 12:
            Math_SmoothStepToAngle(&actor->rot_0F4.x, 218.0f, 0.1f DIV_FRAME_FACTOR, 7.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToAngle(&actor->rot_0F4.y, 147.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->rot_0F4.z, 170.0f, 0.03f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->fwork[0], 20.0f, 0.1f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0f);
            break;
    }
    ...
    if (actor->animFrame == 24) {
        ...
        if (gCsFrameCount > (430 MUL_FRAME_FACTOR)) {
            actor->fwork[29] = 3.0f;
            Math_SmoothStepToF(&actor->rot_0F4.z, 500.0f, 0.1f DIV_FRAME_FACTOR, 20.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&actor->fwork[0], 40.0f, 0.1f DIV_FRAME_FACTOR, 3.0f DIV_FRAME_FACTOR, 0.0f);

            if ((gCsFrameCount < (460 MUL_FRAME_FACTOR)) && (((gCsFrameCount DIV_FRAME_FACTOR) & 3) == 0)) {
...
#if ENABLE_60FPS == 1 // SectorZ_8019E234
void SectorZ_8019E234(Actor* actor) {
    s32 i;

    actor->obj.rot.x += actor->rot_0F4.x DIV_FRAME_FACTOR;
    actor->obj.rot.y += actor->rot_0F4.y DIV_FRAME_FACTOR;
...
#if ENABLE_60FPS == 1 // SectorZ_Boss_Update
void SectorZ_Boss_Update(Boss* boss) {
    ...
    if (!gGreatFoxIntact) {
        ...
        if (((gGameFrameCount % (7 MUL_FRAME_FACTOR)) == 0) && (Rand_ZeroOne() < 0.5f)) {
